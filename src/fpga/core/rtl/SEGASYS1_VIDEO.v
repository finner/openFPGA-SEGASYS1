...
// Sprite Engine — character ROM from Pocket SRAM (full 128KB via sprchdt_ext).
wire [10:0] SPRPX;
wire  [7:0] sprchdt = sprchdt_ext;
...
// Detect real color PROMs (System 2 titles like WBML/Choplifter).
// Clear at download start so System 1 games without PROMs stay on packed RGB8.
reg has_color_prom = 0;
always @(posedge ROMCL) begin
	if (ROMEN && (ROMAD == 25'd0))
		has_color_prom <= 1'b0;
	else if (ROMEN && (`EN_PALR | `EN_PALG | `EN_PALB))
		has_color_prom <= has_color_prom | ~(!ROMDT);
end

// Palette RAM index → PROM RGB for System 2; System 1 stores packed RGB8 in RAM.
// Pack PROM 4-bit channels into Pocket {B[1:0],G[2:0],R[2:0]}.
assign RGB = has_color_prom ? {b[3:2], g[3:1], r[3:1]} : color;
