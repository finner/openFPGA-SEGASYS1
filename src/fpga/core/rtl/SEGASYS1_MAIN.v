...
// Pocket: opcode ROM stubbed (256B). MC8123 key ROM is full 8KB.
DLROM #(15,8) rom0_prg(CLK40M, has_mc8123_key ? CPUAD : rad, rdt, ROMCL,ROMAD,ROMDT,ROMEN & `EN_MCPU0_PRG);
DLROM #(8,8)  rom0_ops(CLK40M, CPUAD[7:0], cpu_rd_mrom0_ops, ROMCL,ROMAD,ROMDT,ROMEN & `EN_MCPU0_OPS);
...
// Detect MC8123 keys / opcode ROMs while downloading.
// Do NOT clear these on RESET — Pocket holds reset for the whole ROM load
// (iRST includes ioctl_download), which would wipe the latch and leave
// encrypted games running raw code (black screen).
reg has_opcode_roms = 0;
reg has_mc8123_key = 0;
always @(posedge CLK40M) begin
	if (ROMEN && (ROMAD == 25'd0)) begin
		has_opcode_roms <= 1'b0;
		has_mc8123_key  <= 1'b0;
	end
	if (ROMEN & `EN_MCPU0_OPS)
		has_opcode_roms <= 1'b1;
	if (ROMEN & `EN_KEY)
